#!/usr/bin/env python
#x20 - ONLY muon decay channel, solid structure of x14: incoprorate PV,SV for ctau preperation
                    ...
                    if mu_gmother.genPartIdxMother == nmu_gmother.genPartIdxMother: #chargino must be the same
                        for neu in neus:
                            ...
                            if mu_gmother.genPartIdxMother == neu_mother.genPartIdxMother: #end point
                                ...
                                chp4 = ch.p4()
                                mag = math.sqrt(math.power(chp4.Px(), 2) + math.power(chp4.Py(), 2) + math.power(chp4.Pz(), 2))
                                bx = chp4.Px()/mag
                                by = chp4.Py()/mag
                                bz = chp4.Pz()/mag
