...
void corr_d_eff_105_pres() {
  ...
  gr_sig->SetTitle("\\mbox{Signal Rejection Efficiency-Displacement dependence } e_{60}; d \\mbox{ (cm)}; e_{60} \\mbox{ (%)}");
  ...
  mg_bkg->SetTitle("\\mbox{Background Rejection Efficiency-Displacement dependence } e_{6j}; d \\mbox{ (cm)}; e_{6j} \\mbox{ (%)}");
  ...
  gr_sig->Draw("AP");
  leg_sig->Draw();
  c_corr->SaveAs("corr_d_eff_sig_105_pres.png");
  gr_rat->Draw("AP");
  leg_rat->Draw();
  c_corr->SaveAs("corr_d_eff_rat_105_pres.png");
  mg_bkg->Draw("AP");
  leg_bkg->Draw();
  c_corr->SaveAs("corr_d_eff_total_105_pres.png");
}
