...
class ExampleDisplacedAnalysis(Module):
    ...
    def beginJob(self, histFile=None, histDirName=None):
        ...
        self.h_mix_chneu_deta = ROOT.TH1F('mix_chneu_deta', '\\mbox{Chargino-Neutralino Delta Eta } \\Delta \\eta', 120, 0, 2.5)
        # GRAPH CUSTOMIZATION
    ...
    def endJob(self):
        ...
        #self.leg_deta = ROOT.TLegend(0.68,0.72,0.98,0.92)
        self.leg_deta = ROOT.TLegend()
        self.leg_deta.AddEntry(self.h_chdeta, "\\tilde{\\chi}_1^\\pm-\\tilde{\\chi}_1^\\pm \\quad \\Delta \\eta" )
        self.leg_deta.AddEntry(self.h_mix_chmu_deta, "\\tilde{\\chi}_1^\\pm-\\mu \\quad \\Delta \\eta")
        self.leg_deta.AddEntry(self.h_mix_chnmu_deta, "\\tilde{\\chi}_1^\\pm-\\nu \\quad \\Delta \\eta")
        self.leg_deta.AddEntry(self.h_mix_chneu_deta, "\\tilde{\\chi}_1^\\pm-\\tilde{\\chi}_1^0 \\quad \\Delta \\eta")
        self.s_deta.Draw()
        self.leg_deta.Draw()
        ...
        self.leg_pt_sus = ROOT.TLegend()
        self.leg_pt_sus.AddEntry(self.h_chpt, "\\tilde{\\chi}_1^\\pm \\quad p_t" )
        self.leg_pt_sus.AddEntry(self.h_neupt, "\\tilde{\\chi}_1^0 \\quad p_t")
        self.s_pt_sus.Draw()
        self.leg_pt_sus.Draw()
        self.c.SaveAs("y" + ver + "/" + "y" + ver + "_h_" + self.s_pt_sus.GetName() + ".png")
        self.c.Update()
        self.leg_pt_sm = ROOT.TLegend()
        self.leg_pt_sm.AddEntry(self.h_mupt, "\\mu \\quad p_t" )
        self.leg_pt_sm.AddEntry(self.h_nmupt, "\\nu \\quad p_t")
        self.s_pt_sm.Draw()
        self.leg_pt_sm.Draw()
