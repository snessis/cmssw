#!/usr/bin/env python
ver = "01"
#cuts: met>=100, >=1 muons, muonpt >= 4, muoneta <=2.5, muondz < 1, muondxy > 0.1
#ctau 10 here
#incoprorate PV
import os, sys, math
    ...
    def analyze(self, event):
        ...
        for jet in Jets:
            if abs(jet.pt) >= 30:
                jets.append(jet)
        for Muon in Muons:
            if genParts[Muon.genPartIdx] in mus and Muon.pt >= 4 and Muon.eta <= 2.5 and METpt >= 100 and Muon.dxy >= 0.25 and Muon.dz <= 1 :
                Mus.append(Muon)
